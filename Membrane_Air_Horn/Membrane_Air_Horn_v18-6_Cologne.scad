...
FONTWIDTHSTRING_BlowCan=[ 0.00,2.50,0.75,2.05,1.85,1.20,1.10,1.45,2.05,2.05,0.75,1.00,1.80,0.50,1.00,0.95,2.05,2.10,1.85,0.95,2.05,1.85,0.50,1.40,2.75,0.50,2.05,0.95,0.75,2.05,2.10,0.95];
...
if (DesignStatus=="textfitting"){
    translate([0,-15,0]){
        TEXTSTRING(STRINGOFTEXT_Cover,FONTWIDTHSTRING_Cover);
    }
    //TEXTSTRING(STRINGOFTEXT_BlowCan,FONTWIDTHSTRING_BlowCan);
    //TEST_Cylinder();
}
...
module TEXTSTRING(STRINGOFTEXT,VECTOR){
    ...
    for(i=[0:len(STRINGOFTEXT)-1]){
        ...
        translate([(Vector_Cover[i]*ONEPointMM*FONTSIZE)+i,0,0.5]){
            color("Fuchsia",0.5){
                linear_extrude(height=EMBOSSDEPTH){
