...
FONT="Liberation Sans:style=Regular";
//Message: 1
STRINGOFTEXT_BlowCan="www.printables.com";  // (18 charackters)
FONTWIDTHSTRING_BlowCan=[ 0.00,2.50,2.50,2.275,0.75,1.85,0.95,0.50,1.85,0.75,1.85,1.85,0.50,1.85,1.625,0.75,1.60,1.850];
    ...
    intersection(){
        ...
        translate([0,0,0]){
            for(i=[0:len(STRINGOFTEXT_EMBOSS)-1]){
                rotate([0,0,Vector_Chooser[i]+Vector_Chooser[i]*0.1]){
                    echo("Vector_Cooser CYLINDER_EMBOSSER",Vector_Cooser);
                    translate([0,0,Spiral_Ascend_Step*i]){
                        rotate([90,0,0]){
                            //rotate([0,0,25]){
                                linear_extrude(height=(OUTER_D/2+2*EMBOSSDEPTH)){
                                    rotate([0,0,Spiral_Ascend_Deg+5.0]){                 // rotates the letters
