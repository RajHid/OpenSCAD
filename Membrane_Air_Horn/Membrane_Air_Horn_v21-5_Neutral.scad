...
module CYLINDER_EMBOSSER(   STRINGOFTEXT_EMBOSS,            // containes the individual letters as a string
                            ...
                            OUTER_D,                        // Outer diameter of the ca
                            FONTSIZE,
                            EMBOSSDEPTH,
    ...
    */
    FooBaa=[for (a = [ 0 : len(FONTWIDTHSTRING_String) - 1 ]) (FONTWIDTHSTRING_String[a]*180*ONEPointMM)*FONTSIZE/(PI*DIAMETER/2) ];
    function cumsum(v) = [
        for (   a = v[0]-v[0], i = 0; i < len(v); a = a+v[i], i = i+1) a+v[i]]; // Some fancy Vector Foo i obviously copy pastet from somewere without deeper understanding
    Vector_Chooser=cumsum(FooBaa);
    ...
    intersection(){
        ...
        translate([0,0,0]){
            for(i=[0:len(STRINGOFTEXT_EMBOSS)-1]){
                rotate([0,0,Vector_Chooser[i]+ROT_Offset]){  // WAS: Vector_Chooser[i]+Vector_Chooser[i]*0.1
                    echo("Vector_Chooser_Ascend CYLINDER_EMBOSSER",Vector_Chooser_Ascend[i]);
                    translate([0,0,tan(Spiral_Ascend_Deg)*Vector_Chooser_Ascend[i]]){  // WAS: Spiral_Ascend_Step*i
                        rotate([90,0,0]){
                            //rotate([0,0,25]){
                                linear_extrude(height=(OUTER_D/2+2*EMBOSSDEPTH)){
                                    rotate([0,0,Spiral_Ascend_Deg]){                 // rotates the letters? 
